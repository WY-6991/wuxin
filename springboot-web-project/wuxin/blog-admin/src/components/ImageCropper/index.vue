  ...
  methods: {
    ...
    checkFile(file) {
      ...
      if (file.type.indexOf('image') === -1) {
        this.hasError = true
        console.log('this.errorMsg.onlyImg====>461 ',lang.error.onlyImg)
    ...
    startCrop() {
      ...
      img.onload = () => {
        ...
        // 图片像素不达标
        if (nWidth < width-200 || nHeight < height-200) {
          this.hasError = true
          this.errorMsg = lang.error.lowestPx + width-200 + '*' + height-200
          console.log('this.errorMsg 516',lang.error.lowestPx + width + '*' + height)
          return false
        }
        if (ratio > nRatio) {
          h = w / nRatio
    ...
    upload() {
      ...
      // console.log('fmData==============='+JSON.stringify(fmData))
      ...
      this.loading = 1
      this.setStep(3)
      request({url, method: 'post', data: fmData}).then(resData => {
          this.loading = 2
          this.$emit('crop-upload-success', resData.result)
        })
        .catch(err => {
          if (this.value) {
            this.loading = 3

<template>
  <div>
    <div
      class="ui attached segment typo content blog"
      v-if="blog"
      :style="bgColor"
    >
      <!--文章是否加密-->
      <div
        class="m-padded-tb-large placeholder"
            ...
            <div class="ui middle aligned mobile reversed">
              <div class="ui grid m-margin-lr">
                ...
                <div class="row m-padded-tb-small">
                  <h2 class="ui header m-center" :style="bgColor">
                    {{ blog.title }}
                  </h2>
                </div>
                <!--文章简要信息-->
                ...
                <router-link
                  ...
                  :class="`ui label  ${
                    blog.category.color ? blog.category.color : 'teal'
                  } ribbon `"
...
import { beforeBlog, nextBlog } from "@/api/blog.js";
...
export default {
  ...
  methods: {
    ...mapActions("blog", ["setBlogId", "getDetailBlog", "getChatUrl"]),
    ...
    loadingContents(loading) {
      ...
      console.log(
        "文章目录加载状态 :>> ",
        this.$store.state.blog.loadingComplete
      );
    },
    ...
    getBeforeBlog() {
      beforeBlog(this.blogId).then((res) => {
        if (res.code === 200) {

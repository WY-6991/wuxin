...
export default {
  ...
  data() {
    const validEmailCheck = (rule, value, callback) => {
      if (!validEmail(value)) {
        callback(Error("邮箱格式不正确!"));
      }
      callback();
    };
    return {
  ...
  methods: {
      ...
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          this.$emit("addComment", {
            'username': this.commentUser.username,
            'email': this.commentUser.email,
            'content': this.commentUser.content,
            'subscription': this.commentUser.subscription,
            'blogId': this.id,
            'type': this.type,
          });
        }
    ...
    submitReply() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          if (!this.commentId) {
            return this.$message.error("评论不存在！");
          }
          if (!this.commentUserId || this.commentUserId === "") {
            return this.$message.error("发布失败，获取不到用户");
          }
          ...
          this.$emit("addReply", {
            ...
            'replyContent': this.commentUser.content,
            'subscription': this.commentUser.subscription,
